...
plot average ER from available ng plants from the generator data objects from simple dispatch
...
if __name__ == '__main__':
    ...
    #%%
    i = 0 # choose which site to run for
    ...
    ER_so2_beg = pd.DataFrame()
    ER_nox_all = pd.DataFrame()
    ER_nox_beg = pd.DataFrame()
    
    ## import generator data for 2006
    os.chdir(base_dname)
    os.chdir(rel_path_input_generators)
    gen_data_beg = pd.DataFrame()
    for ba_region in ba_region_group:
        gd_short = pickle.load(open('generator_data_short_%s_%s.obj'%(ba_region, str(2006)), 'rb')) # load generatordata object
        gen_data_beg = pd.concat([gen_data_beg, gd_short['df']], axis=0) # load generator data dataframe and concat to existing gen data (if multiple)
    ...
    for run_year in years_all:
        ...
        mask = (gen_data['fuel'] == 'ng') & (gen_data['prime_mover'].isin(['cc', 'ct'])) # 3
        ...
        ### calculate metrics for only EGUs that are active in the beginning of time period
        ## retrieve operating EGUs
        # index for natural gas CC EGUs that are present in beginning
        mask = (gen_data_beg['fuel'] == 'ng') & (gen_data_beg['prime_mover'].isin(['cc', 'ct']))
        # obtain operating EGU orispl_unit
        EGU_operating = gen_data_beg.loc[mask, 'orispl_unit']
        ...
        ER_nox_year = calc_weighted_metrics(gen_data_operating, run_year, 'nox')
        ## concatenate to final dataframe
        ER_so2_beg = pd.concat([ER_so2_beg, ER_so2_year], axis=0)
        ER_nox_beg = pd.concat([ER_nox_beg, ER_nox_year], axis=0)
    ...
    ax.plot(dates, ER_so2_beg['average'], color='green', label='EGUs present since beginning')
    ...
    ax.fill_between(dates, ER_so2_beg['average'] - ER_so2_beg['std'],
                    ER_so2_beg['average'] + ER_so2_beg['std'], color='lightgreen', alpha=0.5)
    ...
    ax.plot(dates, ER_nox_beg['average'], color='green', label='EGUs present since beginning')
    
    # Plot the std shading
    ax.fill_between(dates, ER_nox_all['average'] - ER_nox_all['std'],
                    ER_nox_all['average'] + ER_nox_all['std'], color='lightblue', alpha=0.5)
    ax.fill_between(dates, ER_nox_beg['average'] - ER_nox_beg['std'],
                    ER_nox_beg['average'] + ER_nox_beg['std'], color='lightgreen', alpha=0.5)
    ...
    with pd.ExcelWriter('ER_trends_ng_'+ region_names[i] +'.xlsx') as writer:
        # so2
        ER_so2_all.to_excel(writer, sheet_name='so2 all', index=False)
        ER_so2_beg.to_excel(writer, sheet_name='so2 beg', index=False)
        # nox
        ER_nox_all.to_excel(writer, sheet_name='nox all', index=False)
        ER_nox_beg.to_excel(writer, sheet_name='nox beg', index=False)